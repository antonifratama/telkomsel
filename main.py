...
def log(value, status='INFO Local Port', color='[R1]'):
...
class inject(object):
    ...
    def start(self):
        try:
            ...
            frontend_domains = open(real_path('/vclife.txt')).readlines()
            frontend_domains = filter_array(frontend_domains)
            if len(frontend_domains) == 0:
                self.log('Frontend Domains not found. Please check vclife.txt', color='[Y1]')
                return
...
class domain_fronting(threading.Thread):
    ...
    def run(self):
        try:
            ...
            self.log('Vclifetv Mengambil Jaringan'.format(self.proxy_host, self.proxy_port))
            ...
            self.log('Vclifetv Gagal Mengambil ''[R2]'.format(self.proxy_host, self.proxy_port), color='[R1]')
        except OSError:
...
def main():
    print(colors('\n'.join([
        ...
        '',
        '[CC]'
        '[R1]BAKUHANTAM CREW', '[CC]'
        '[P1]KUNCHEN ID', '[CC]'
        '[P1]INTERNET GRATIS ', '[CC]'
        '[G1]TELKOMSEL 2019',
        'PLEASE DONT REPORT THIS IS SCRIPT',
        '',
    ])))
